---
section: "09-financials"
slug: "financial-forecast"
title: "Financial Forecast — 24-Month Lean View"
---

**Luvian Labs LLC · Pre-Seed · 2026-05 · Confidential**

> Lean by design. Pre-revenue. Burn-driven. Milestone-anchored. We
> deliberately do **not** ship a 5-year line-itemised "enterprise"
> projection model with fake precision — at pre-seed it is theatre.
> Investors evaluate *capital awareness*, not *spreadsheet artistry*.

The companion document is `Use_of_Funds_2026_05.md` (the round-shape
view). This document is the *runway* view.

---

## 1. Headline numbers

| Metric | Value |
|---|---|
| Round target | $1.5M – $2.0M (Convertible Note) |
| Runway at planned spend ($1.75M) | **18 months** |
| Runway at lean spend ($1.5M) | ~22 months |
| Average burn at planned (peak team) | ~$104K/month |
| Pre-revenue throughout the planning horizon | Yes (revenue is bonus, not budget) |
| Series A target | $8M–$15M priced round, 12–18 months from pre-seed close |

We do not budget revenue from design partners as runway. Design-partner
revenue is treated as bonus runway, not as a planned line in the
forecast — pre-seed customers convert irregularly and tying runway to
that conversion produces planning errors.

---

## 2. 18-month burn shape

| Quarter | Months | Avg burn | Cum. burn | Triggers |
|---|---|---:|---:|---|
| Q1 | 1–3 | ~$60K/mo | ~$180K | Round close, co-founder hire #1 (eng), legal flip kickoff |
| Q2 | 4–6 | ~$95K/mo | ~$465K | Co-founder hire #2 (GTM), 2 senior engineers, ingestion ramp |
| Q3 | 7–9 | ~$115K/mo | ~$810K | First design partner in production; founding designer; on-prem packaging |
| Q4 | 10–12 | ~$120K/mo | ~$1,170K | Second design partner; Delaware Flip complete |
| Q5 | 13–15 | ~$120K/mo | ~$1,530K | Third design partner; Series-A-credible architecture story |
| Q6 | 16–18 | ~$110K/mo | ~$1,860K | Series A prep + close |

End of Q6 cumulative: **~$1.86M**, within the $1.5–$2.0M range
(midpoint $1.75M).

---

## 3. Burn composition (steady state, post month 6)

| Bucket | % of burn |
|---|---:|
| Salaries + benefits + payroll tax (6 people) | ~70% |
| Legal, accounting, ops | ~5% |
| Compute + foundation-model API + infra | ~7% |
| Tooling, software subscriptions | ~5% |
| Office / co-working / travel | ~5% |
| Recruiting, contractor coverage, advisor stipends | ~5% |
| Buffer / contingency | ~3% |

> Once the team is fully assembled (~month 6), salaries dominate. This
> is normal for pre-seed infra and is the right shape — pre-seed
> dollars convert primarily into team.

---

## 4. Sensitivity — what changes the picture

### Faster co-founder hires (months 1–2)

If both co-founders close in the first 60 days, Q2 burn picks up
earlier, but milestone ramp also accelerates by ~1 quarter — Series A
prep starts in month 14 rather than month 16. Net runway impact:
neutral; net Series A timing: positive.

### Slower co-founder hires (months 4–6)

If co-founder hires slip by 2–3 months, Q2 burn is lower than planned
(extending runway by ~1.5 months) but **milestone risk increases** —
the architecture-doc and design-partner-deployment milestones depend
on the co-founder team. Net runway impact: positive; net milestone
risk: meaningful.

This is the highest-leverage sensitivity. The lead investor's
recruiting help directly de-risks it.

### One design partner converts to paid before Series A

At pre-seed it is plausible that one of the 2–3 design partners signs
a paid pilot ($25K–$150K range) before the Series A closes. We do
**not** budget this. If it lands, it is bonus runway and stronger
Series-A material — neither the round size nor the milestone plan
depend on it.

### Faster compute/model price decline

Foundation-model API costs have been declining 50–70% per year for
equivalent capability since 2023. We model the compute + model bucket
as flat for safety; the realistic outcome is that this bucket shrinks
over the round, contributing 1–2 months of additional runway.

---

## 5. Financial controls

- **Monthly P&L** to the lead investor (and any SAFE-equivalent
  investors, on request).
- **Quarterly milestone review** against the milestone plan in
  `Use_of_Funds_2026_05.md`.
- **Hiring commits** are made only after the first design-partner
  technical commitment (gate at month 6).
- **No raise above $2.0M at this stage** — see `Use_of_Funds_2026_05.md`,
  §"Bridge / runway scenarios," for why.

---

## 6. Series A bridge planning

The Series A is contemplated **12–18 months** from pre-seed close,
priced into the post-Flip Delaware C-corp. Target round size: $8M–$15M.

The key Series-A trigger is **architecture + reference customers**, not
revenue:

- 2–3 design partners in production with provenance + suspect-link
  propagation visible.
- Reference architecture publicly documented and battle-tested at the
  partners.
- 6-person team operating without the founder as the bottleneck on any
  workstream.
- Clean post-Flip cap table.

Pre-seed investors' pro-rata participation in the Series A is a
contemplated and capital-planned outcome — see Investor Q&A, Q15.

---

## 7. What we are honest about

- This is a **plan**, not a guarantee. Pre-seed plans are revisited
  every 90 days against actual milestone progress.
- The round size is **deliberately conservative**. We are sized to be
  rational, not to be impressive.
- The runway buffer is **deliberately tight**. We are not raising for
  optionality; we are raising for execution.
- We are **pre-revenue throughout the planning horizon**. We do not
  plan around revenue we have not signed.

---

— *Stephan Claxton, Founder, Luvian Labs LLC* · 2026-05-21
